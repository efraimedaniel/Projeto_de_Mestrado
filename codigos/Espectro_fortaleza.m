...
%hold on
%plot(tempo_numerica, tmp,'r')
%grid

figure;
plot(tempo_numerica, altura_detrend);
title('Magnitude da FFT dos dados de altura_detrend');
xlabel('Frequência (cycles/month)');
ylabel('Magnitude');
...
%ylabel('Densidade espectral (m^2 \times meses)', 'FontSize', 12); % Unidades específicas

%title('Análise espectral do nível médio do mar em Fortaleza', 'FontSize', 14);
...
%print('Espectro_Fourier_Tidal_Signal_Fortaleza', 'svd', '-r300');
% Salvar a figura no formato SVG com 300 DPI
print('Espectro_Fourier_Tidal_Signal_Fortaleza_semtitulo', '-dsvg', '-r300');
