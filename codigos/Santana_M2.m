...
coef = ut_solv(data_numerica, altura_zero, [], -0.061389, 'auto');
...
for i = 1:length(anos_unicos)
    ...
    coeficientes_harmonicos{i} = ut_solv(data_numerica(indices_ano_atual), ...
                                         alturas_ano_atual, [], -0.061389, 'auto');
...
for i = 1:length(anos_unicos)
    ...
    indice_M2 = find(strcmp(coeficientes_atual.name, 'S2'));
...
title('Amplitude da Componente M2 em Santana');
grid on;
...
title('Fase da Componente M2 em Santana');
grid on;
%}
...
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(amplitudes_M2);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
amplitudes_M2_limpo = amplitudes_M2(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_M2 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_M2 = vetor_tmp_M2\amplitudes_M2_limpo;
b1_M2 = vetor_tmp_M2 * x1_M2;
...
%%
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(amplitudes_S2);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
amplitudes_S2_limpo = amplitudes_S2(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_S2 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_S2 = vetor_tmp_S2\amplitudes_S2_limpo;
...
%%
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(amplitudes_O1);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
amplitudes_O1_limpo = amplitudes_O1(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_O1 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_O1 = vetor_tmp_O1\amplitudes_O1_limpo;
...
%%
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(amplitudes_M3);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
amplitudes_M3_limpo = amplitudes_M3(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_M3 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_M3 = vetor_tmp_M3\amplitudes_M3_limpo;
...
%
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(fases_M2);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
fases_M2_limpo = fases_M2(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_M2 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_M2 = vetor_tmp_M2\fases_M2_limpo;
b1_fase_M2 = vetor_tmp_M2 * x1_M2;
%
...
%%
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(fases_S2);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
fases_S2_limpo = fases_S2(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_O1 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_S2 = vetor_tmp_S2\fases_S2_limpo;
b1_fase_S2 = vetor_tmp_S2 * x1_S2;
%
...
%%
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(fases_O1);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
fases_O1_limpo = fases_O1(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_O1 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_O1 = vetor_tmp_O1\fases_O1_limpo;
b1_fase_O1 = vetor_tmp_O1 * x1_O1;
...
%%
% Encontrar índices de valores NaN em amplitudes_M2
nan_indices = isnan(fases_M3);

% Remover valores NaN de anos_unicos e amplitudes_M2
anos_unicos_limpo = anos_unicos(~nan_indices);
fases_M3_limpo = fases_M3(~nan_indices);

% Criar uma tendência para as amplitudes limpas
vetor_tmp_M3 = [ones(length(anos_unicos_limpo), 1) anos_unicos_limpo];
x1_M3 = vetor_tmp_M3\fases_M3_limpo;
b1_fase_M3 = vetor_tmp_M3 * x1_M3;
%
...
subplot(2, 4, 1);
plot(anos_unicos_limpo, amplitudes_M2_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_M2, 'LineWidth', 2);
title('Amplitude M2 em Santana');
xlabel('Ano');
...
%%
subplot(2, 4, 2);
plot(anos_unicos_limpo, amplitudes_S2_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_S2, 'LineWidth', 2);
title('Amplitude S2 em Santana');
xlabel('Ano');
...
%%
subplot(2, 4, 3);
plot(anos_unicos_limpo, amplitudes_O1_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_O1, 'LineWidth', 2);
title('Amplitude O1 em Santana');
xlabel('Ano');
...
subplot(2, 4, 4);
plot(anos_unicos_limpo, amplitudes_M3_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_M3, 'LineWidth', 2);
title('Amplitude M3 em Santana');
xlabel('Ano');
ylabel('Amplitude da M3 (cm)');
%legend('Amplitude da M3', 'Tendência da K1', 'FontSize', 8);
grid on;
...
subplot(2, 4, 5);
plot(anos_unicos_limpo, fases_M2_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_fase_M2, 'LineWidth', 2);
title('Fase da M2 em Santana');
xlabel('Ano');
ylabel('Fase (graus)');
...
%%
subplot(2, 4, 6);
plot(anos_unicos_limpo, fases_S2_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_fase_S2, 'LineWidth', 2);
title('Fase da S2 em Santana');
xlabel('Ano');
ylabel('Fase (graus)');
...
%%
subplot(2, 4, 7);
plot(anos_unicos_limpo, fases_O1_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_fase_O1, 'LineWidth', 2);
title('Fase da O1 em Santana');
xlabel('Ano');
ylabel('Fase (graus)');
...
%%
subplot(2, 4, 8);
plot(anos_unicos_limpo, fases_M3_limpo, 'o-', 'LineWidth', 2);
hold on
plot(anos_unicos_limpo, b1_fase_M3, 'LineWidth', 2);
title('Fase da M3 em Santana');
xlabel('Ano');
ylabel('Fase (graus)');
...
%saveas(gcf, 'Componente_M2_Santana.png');
...
xlim([2005, 2022]);
ylim([1.05 1.20]);
yticks(1.0:0.05:1.20);
%xlabel('Ano');
ylabel('Amplitude (cm)');
grid on;
% Adicionar legenda com rótulos explicativos
legend('M2 (A)', 'Location', 'northwest', 'FontSize', 5);
...
y_pos = 1.05; %0.947; % Posição y igual ao valor mínimo do eixo y
...
%xlabel('Ano');
ylabel('Fase (graus)');
legend('M2 (B)', 'Location', 'southwest', 'FontSize', 5);
grid on;

% Ajuste das coordenadas para a posição do texto
x_pos = 2022;  % Posição x igual ao valor máximo do eixo x
y_pos = 114; %0.0102; % Posição y igual ao valor mínimo do eixo y
...
% Adicionar texto com a taxa anual de variação da amplitude da componente M2
text(x_pos, y_pos, sprintf('= %.1e cm.ano^{-1}', var_b1_linear_S2),...
    'HorizontalAlignment', 'right', 'VerticalAlignment', 'bottom', 'FontSize', 8)
...
hold on
plot(anos_unicos, b1_fase_S2, 'b', 'LineWidth', 0.5);
...
% Ajuste das coordenadas para a posição do texto
x_pos = 2022;  % Posição x igual ao valor máximo do eixo x
y_pos = 135;  %0.0102; % Posição y igual ao valor mínimo do eixo y
...
xlim([2005, 2022]);
ylim([0.040 0.060]);
yticks(0.040:0.02:0.060);
%xlabel('Ano');
ylabel('Amplitude (cm)');
legend('O1 (E)', 'Location', 'southwest', 'FontSize', 5);
grid on;

% Calcular as coordenadas x e y para o canto inferior direito do subplot
x_pos = 2022;  % Posição x igual ao valor máximo do eixo x
y_pos = 0.040; %0.0693; % Posição y igual ao valor mínimo do eixo y
...
hold on
plot(anos_unicos, b1_fase_O1,'b', 'LineWidth', 0.5);
xlim([2005, 2022]);
ylim([340 360]);
yticks(340:10:360);
...
%xlabel('Ano');
ylabel('Fase (graus)');
legend('O1 (F)', 'Location', 'southwest', 'FontSize', 5);

grid on;

% Ajuste das coordenadas para a posição do texto
x_pos = 2022;  % Posição x igual ao valor máximo do eixo x
y_pos = 340; %0.0102; % Posição y igual ao valor mínimo do eixo y
% Adicionar texto com a taxa anual de variação da amplitude da componente M2
...
xlabel('Anos');
ylabel('Amplitude (cm)');
legend('M3 (G)', 'Location', 'southwest', 'FontSize', 5);
grid on;

x_pos = 2022;  % Posição x igual ao valor máximo do eixo x
y_pos = 0 %0.0102; % Posição y igual ao valor mínimo do eixo y
...
hold on
plot(anos_unicos, b1_fase_M3, 'b', 'LineWidth', 0.5);
xlim([2005, 2022]);
ylim([100 350]);
yticks(100:200:350);
...
xlabel('Anos');
ylabel('Fase (graus)');
legend('M3 (H)', 'Location', 'southwest', 'FontSize', 5);
grid on;

% Ajuste das coordenadas para a posição do texto
x_pos = 2022;  % Posição x igual ao valor máximo do eixo x
y_pos = 100; %0.0102; % Posição y igual ao valor mínimo do eixo y
...
%print('Componente_M2_Santana_01.png', '-dpng', '-r300');
...
% Salva a figura em WMF
print('Componente_M2_Santana_02.wmf', '-dmeta');

%}
% Título global da figura
%sgtitle('Análise das Componentes Harmônicas das Marés em Santana', ...
...
fig.PaperSize = [21 13]; % Define o tamanho do papel como 21x13 cm
fig.PaperPosition = [0 0 21 13]; % A figura ocupa todo o papel
print('Componente_M2_Santana_03.svg', '-dsvg', '-r300');
